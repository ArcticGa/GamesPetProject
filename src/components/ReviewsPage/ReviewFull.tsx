...
const ReviewFull = ({ review }: { review: IReview }) => {
	const published = useGetDate(review.published_date)
		...
		<div className='flex flex-col p-4 bg-main-blocks rounded-2xl max-w-[480px]'>
			<div className='flex items-center justify-between rounded-2xl bg-main-background shadow-lg px-6 py-3 mb-4'>
				<div className='flex items-center'>
					<img className='mr-2' src={UserAvatar} alt='user-avatar' />
					<div>
						<div className='font-bold text-lg'>Monke</div>
						<div className='text-xs text-[#c5c5c5]'>Обзоров: 2</div>
					</div>
				</div>
				<div className='flex items-center bg-main-blocks text-sm px-4 py-2 rounded-xl'>
					<img
						src={review.is_recommended ? RedHeartIcon : HeartIcon}
						alt='heart-icon'
					/>
					<div className='ml-2'>
						{review.is_recommended ? 'Рекомендую' : 'Не рекомендую'}
					</div>
				</div>
