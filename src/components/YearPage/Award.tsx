...
const Award = ({ data, gamesYear }: IAwardProps) => {
	...
	return !winner ? (
		...
		<div
			className='p-10 rounded-2xl mb-5 max-w-[1061px] w-full max-md:p-4'
			style={{
				backgroundImage: `linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.6)), url('${data.bgImage}')`,
				backgroundRepeat: 'no-repeat',
			}}
		>
			<div className='flex justify-between items-center max-md:flex-col max-lg:items-stretch'>
				<div className='flex flex-col items-start mr-4 mb-4'>
					<div className='text-lg max-w-[350px]'>
						Премия «{data.titleAward}»
					</div>
					<div className='text-3xl font-bold py-2 pb-3'>{winner.name}</div>
					<div className='max-w-[385px] flex-1 text-sm text-[#d7d7d7] mb-4'>
						{winner.description}
					</div>
					<div
				...
				<div className='relative rounded-2xl shadow-md max-lg:self-center'>
					<img
					...
					{data.id === 1 && (
						<img
							className='absolute -top-10 -right-10 rotate-45 w-20 max-lg:hidden'
							src={CrownIcon}
							alt='crown-icon'
						/>
					)}
			...
			{openFinalists && (
				<div className='mt-6'>
					<div className='flex items-center mb-3'>
						<div className='mr-2 max-md:text-xs'>Финалисты:</div>
						<hr className='w-full' />
					</div>
